import Image from 'next/image'
import React from 'react'
import Button from './Button'


const Hero = () => {
  const phoneNumber = '+91 9418454877';
  return (
    <section className="max-container padding-container flex flex-col gap-20 py-10 pb-32 md:gap-28 lg:py-20 xl:flex-row ">
      <div className='hero-map' />
      {/*LEFT SIDE*/}
      <div className='relative z-20 flex flex-col xl:w-1/2'>
      <Image 
      src="/camp.svg"
      alt='camp'
      width={50}
      height={50} 
      className='absolute left-[-5px] top-[-30px] w-10 lg:w-[50px]'
      />
      <h1 className='bold-52 lg:bold-88'>Cozy Haven Homestay</h1>
      <p className='regular-16 mt-6 text-gray-30 xl:max-w-[520px]'>
      Discover a sanctuary in the heart of Shimla, 
      where modern comfort meets serene tranquility. 
      Nestled amidst the picturesque landscapes, Cozy Haven offers an intimate retreat away from the hustle and bustle.
      </p>
      {/*Stars are put here*/}
      <div className='my-11 flex flex-wrap gap-5'>
        <div className='flex items-center gap-2'>
          {Array(5).fill(1).map((_, index) => (
            <Image 
            src="/star.svg"
            key={index}
            alt='star'
            width={24}
            height={24} />
          ))}

        </div>
        <p className='bold-16 lg:bold-20 text-blue-70'>Excellent Reviews 
          <span className='regular-16 lg:regular-20 ml-1  '>Coming Soon!</span>
        </p>
      </div>
      <div className='flex flex-col w-full gap-3 sm:flex-row'>
        <Button type='button' title='Book a Room' variant="btn_green" href={`tel:${phoneNumber}`}/>
        <Button type='button' title='Explore' icon='/play.svg' variant="btn_white_text"/>

      </div>
      </div>
      <div className='relative flex flex-1 items-start'>
         <div className='relative z-20 w-[268px] flex flex-col gap-8 rounded-3xl bg-green-90 px-7 py-8 lg:-translate-x-16 '>
          <div className='flex flex-col'>
            <div className='flexBetween'>
              <p className='regular-16 text-gray-20'>Location</p><Image src="/close.svg" alt='close' width={24} height={24} />
              
              </div>
              
              <p className='bold-20  text-white'>Mehli, Shimla</p>
              </div>
          
            <div className='flexBetween'>
              <div className='flex flex-col'>
                <p className='regular-16 text-gray-20 block'>Distance</p>
                <p className='bold-20 text-white'>7 km</p>

              </div>
              <div className='flex flex-col'>
                <p className='regular-16 text-gray-20 block'>Elevation</p>
                <p className='bold-20 text-white'>2.296 km</p>

              </div>

            </div>

          

         </div>
      </div>

    </section>

    
  )
}

export default Hero
